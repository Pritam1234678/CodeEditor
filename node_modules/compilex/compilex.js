    ...
    fs.readdir(path, function (err, files) {
        if (!err) {
            for (let i = 0; i < files.length; i++) {
                fs.rm(path + files[i], { recursive: true, force: true }, (err) => {
                    if (err) {
                        console.warn("Could not delete file:", path + files[i], err.message);
                    }
                });
            }
        }
    });
    fn(); // continue even if some files weren't deleted
}
